...
namespace NexgenCosysReport.Services.ReportService
{
    public class JsReportService : IJsReportService
    {
        ...
        public async Task<byte[]> ExportChunkedPdfAsync(
            string reportKey,
            string reportPath,
            ...
            CancellationToken ct = default)
        {
            // ── Final PDF cache check ──────────────────────────────────────
            if (TryGetCachedPdf(reportKey, out var cached) && cached != null)
            {
                _logger.LogInformation("📦 Chunked PDF cache hit — {Key}", reportKey);
                return cached;
            }
            ...
            // ── Render chunks (bounded parallelism) ────────────────────────
            ...
            for (int i = 0; i < chunks.Count; i++)
            {
                ...
                tasks.Add(Task.Run(async () =>
                {
                    try
                    {
                        ...
                        var html = await RenderRazorViewToStringAsync(reportPath, chunkData).ConfigureAwait(false);
                        var bytes = await RenderHtmlToBytesAsync(html, "PDF", pageSetting, ct).ConfigureAwait(false);

                        results[idx] = bytes;

                        _logger.LogInformation("✅ Chunk {Idx}/{Total} done — {MB:F2} MB",
                            idx + 1, chunks.Count, bytes.Length / 1024.0 / 1024.0);
                    }
                    finally { sem.Release(); }
                }, ct));
            }
            ...
            // ── Merge + compress ───────────────────────────────────────────
            _logger.LogInformation("🔗 Merging {Count} chunks...", results.Length);
            var merged = PdfChunkMerger.MergeChunks(results, _logger);
            var compressed = ReportUtils.CompressPdf(merged, _logger);

            _logger.LogInformation("✅ Final merged PDF — {MB:F2} MB",
                compressed.Length / 1024.0 / 1024.0);

            CachePdf(reportKey, compressed);
            return compressed;
        }
            ...
            string html, string format, PageSizeSetting? pageSetting, CancellationToken ct)
        {
            ...
            ConfigureJsReportTemplate(request, format, pageSetting);
        ...
        private static void ConfigureJsReportTemplate(
            RenderRequest request, string format, PageSizeSetting? pageSetting)
        {
            switch (format)
            {
                case "PDF":
