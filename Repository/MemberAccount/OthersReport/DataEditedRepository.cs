...
//                string branchName = officeIds == "-1" ? "All" : await GetBranchNamesByIdsAsync(officeIds);
...
//        /// list of office/branch names. Falls back to the raw id list if the lookup fails
//        /// or a given id has no matching name.
//        /// </summary>
//        private async Task<string> GetBranchNamesByIdsAsync(string officeIdsCsv)
//        {
//            try
//            {
...
//                // NOTE: adjust table/column names (UsmOffice / OfficeName / UsmOfficeId)
//                // if your schema names them differently.
...
//                return names.Any() ? string.Join(", ", names) : officeIdsCsv;
//            }
//            catch (Exception ex)
//            {
...
// Repository/MemberAccount/OthersReport/DataEditedReportRepository.cs
using Dapper;
using Microsoft.Data.SqlClient;
using Microsoft.EntityFrameworkCore;
using NexgenCosysReport.DbContext;
    ...
    public class DataEditedReportRepository : IDataEdited
    {
        ...
        public async Task<DataEditedReportData> GetReportDataAsync(DataEditedReportRequestDto request)
        {
            try
            {
                ...
                string? entryByName = entryBy != -1 ? await GetUserNameByIdAsync(entryBy) : null;
                string? editedByName = editedBy != -1 ? await GetUserNameByIdAsync(editedBy) : null;
                string branchName = await _branchNameResolver.GetBranchNamesAsync(officeIds, allLabel: "All");
