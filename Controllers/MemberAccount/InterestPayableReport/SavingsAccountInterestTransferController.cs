...
using NexgenCosysReport.Interfaces.ServiceInterface.MemberAccount.SavingsAccountInterestTransfer;
...
namespace NexgenCosysReport.Controllers.MemberAccount.SavingsAccountInterestTransfer
{
    ...
    public class SavingsAccountInterestTransferController : ControllerBase
    {
            ...
            [FromQuery] string format = "VIEW")
        {
            try
            {
                ...
                if (!data.Rows.Any())
                {
                    return NotFound(new GeneralResponse<ReportResponseDtos>
                    {
                        isValid = false,
                        statusCode = 404,
                        message = "No savings account interest transfer records found for the selected criteria"
                    });
                }
            ...
            {
                _logger.LogError(ex, "Savings Account Interest Transfer report generation failed");
