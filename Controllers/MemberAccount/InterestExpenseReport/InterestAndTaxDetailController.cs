    ...
    public class InterestAndTaxDetailController : ControllerBase
    {
            ...
            [FromQuery] string format = "VIEW")
        {
            try
            {
                ...
                // Validate request (converted from btnViewByDate_Click validation)
                ...
                // Check if member exists (converted from txtMemberId_TextChanged)
                if (!string.IsNullOrEmpty(request.MemberId) && request.MemberId != "-1")
                {
            ...
            {
                _logger.LogError(ex, "Interest and Tax Detail report generation failed");
