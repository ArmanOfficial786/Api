// Controllers/Microfinance/MicrofinanceReport/LoanTypeWiseController.cs
...
namespace NexgenCosysReport.Controllers.Microfinance.MicrofinanceReport
{
    ...
    [Authorize]
    public class LoanTypeWiseController : ControllerBase
    {
        private readonly ILoanTypeWiseRepository _repository;
        private readonly ICommonHeaderRepository _commonHeaderRepository;
        private readonly IJsReportService _jsReportService;
        private readonly IWebHostEnvironment _webHostEnvironment;
        private readonly CustomHeaderResponse _headerResponse;
        private readonly IOptions<ReportSettings> _reportSettings;
        private readonly ILogger<LoanTypeWiseController> _logger;

        public LoanTypeWiseController(
            ILoanTypeWiseRepository repository,
            ICommonHeaderRepository commonHeaderRepository,
            IJsReportService jsReportService,
            IWebHostEnvironment webHostEnvironment,
            CustomHeaderResponse headerResponse,
            IOptions<ReportSettings> reportSettings,
            ILogger<LoanTypeWiseController> logger)
        {
        ...
        public async Task<IActionResult> GenerateReport(
            [FromBody] LoanTypeWiseRequestDto request,
            [FromQuery] string format = "VIEW")
        {
            try
            {
                ...
                var reportData = new Dictionary<string, object>
                {
                    ...
                    { "TotalOpeningDisburseAmount", data.TotalOpeningDisburseAmount },
                    ...
                    { "LoanGuarantee", data.LoanGuarantee ?? "-1" },
                    { "ShareType", data.ShareType ?? "-1" },
                    { "OrderBy", data.OrderBy ?? "-1" },
                    { "SameCompanyName", data.SameCompanyName },
                    { "ShowOpeningBalance", data.ShowOpeningBalance },
                    { "ShowDetail", data.ShowDetail },
                    { "GroupByBranch", data.GroupByBranch },
                    { "GroupByCollectionCenter", data.GroupByCollectionCenter },
                    { "GroupByMemberGroup", data.GroupByMemberGroup },
                    { "ViewCollector", data.ViewCollector },
                    { "Format", upperFormat }
                };

                string viewPath = request.VisualReport
                    ? "Views/VisualReport/Microfinance/VLoanTypeWiseReport.cshtml"
                    : "Views/Report/Microfinance/MicrofinanceReport/LoanTypeWiseReport.cshtml";
            ...
            {
                _logger.LogError(ex, "Error generating LoanTypeWise report");
